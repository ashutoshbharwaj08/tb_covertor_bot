...
bot = TelegramClient("tele", 27083228, 55ea6ac2b88a211c0b9f9b3387ea076b)
...
Join @teraboxbot For Updates
"""
    check_if = await is_user_on_chat(bot, "@teraboxbot", m.peer_id)
    if not check_if:
        return await m.reply("Please join @teraboxbot then send me the link again.")
    check_if = await is_user_on_chat(bot, "@teraboxbotchat", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @teraboxbotChat then send me the link again."
        )
...
Size: **{data["size"]}** 
Direct Link: [Click Here](https://t.me/teraboxbot123_bot?start={uuid})
